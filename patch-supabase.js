...
content = content.replace(
  /recipient_account_id: null, \/\/ External account, not in our database\s+\}\)\s+\.select\(\)/,
...
content = content.replace(
  /if \(parseFloat\(sourceAccountData\.balance\.toString\(\)\) < amount\) throw new Error\('Insufficient funds'\)\s+const fullDescription = bankDetails/,
...
content = content.replace(
  /if \(parseFloat\(sourceAccountData\.balance\.toString\(\)\) < amount\) throw new Error\('Insufficient funds'\)\s+const fullDescription = bankDetails/g,
  `if (parseFloat(sourceAccountData.balance.toString()) < amount) throw new Error('Insufficient funds')
...
console.log('supabase-services.ts patched successfully');
